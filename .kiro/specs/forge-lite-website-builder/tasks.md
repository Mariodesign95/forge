...
- [ ] 1. Setup progetto e struttura base
  - [ ] 1.1 Inizializza il progetto Electron + React + TypeScript + Vite
    ...
    - Configura `package.json` root con `electron-builder`, dipendenze principali: `electron`, `electron-builder`, `electron-updater`, `electron-store`, `node-machine-id`, `archiver`, `ai`, `@ai-sdk/anthropic`, `@ai-sdk/openai`, `@ai-sdk/google`, `react`, `react-dom`, `@monaco-editor/react`, `react-resizable-panels`, `fast-check`, `vitest`, `@testing-library/react`
...
- [ ] 2. Implementa KeyStore e Settings persistenza
  ...
  - [ ] 2.3 Implementa IPC handlers per keys e settings
    - Crea `src/main/ipc-handlers.ts` (o aggiunge i handler): registra `forge:keys:save`, `forge:keys:get-masked`, `forge:keys:delete`
    - Implementa `src/main/project-store.ts`: gestisce `appStore` (settings, tema, layout), `projectListStore` (lista progetti, LRU max 20)
...
- [ ] 3. Implementa ModelRouter (Vercel AI SDK)
  ...
  - [ ] 3.3 Implementa IPC handler per `forge:ai:chat` e `forge:ai:abort`
    - In `src/main/ipc-handlers.ts`: registra `forge:ai:chat` → chiama `ModelRouter.stream()`, itera sull'AsyncIterable e invia `forge:ai:stream` chunks via `webContents.send()`
    - Alla fine dello stream, invia `{ done: true }`
    - Registra `forge:ai:abort` → chiama `AbortController.abort()` del job corrente
    - Mantiene una `Map<string, AbortController>` per jobId attivi
...
- [ ] 6. Implementa ProjectStore e gestione filesystem
  - [ ] 6.1 Implementa `ProjectStore` con electron-store e filesystem
    - Crea `src/main/project-store.ts` (se non già avviato nel task 2.3)
